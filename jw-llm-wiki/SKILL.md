...
description: |
  ...
  自动整理为结构化的 wiki。
  触发条件：用户明确提到"知识库"、"wiki"、"jw-llm-wiki"，或要求对已初始化的知识库执行
...
- `overview-template.md` — 总览模板
- `purpose-template.md` — 研究方向模板
...
- `wechat-article-to-markdown` — 微信公众号提取（⚠️ 安装 pitfall 见 `references/wechat-install-pitfall.md`）
...
4. **运行初始化脚本**：
   ...
   > **Pitfall**：`init-wiki.sh` 依赖 `templates/` 下的 5 个模板文件（schema-template.md、index-template.md、log-template.md、overview-template.md、purpose-template.md）。如果这些文件缺失，脚本会创建空文件。运行后必须检查生成的文件是否为空，若为空则手动填充内容（参考模板文件格式）。
