...
**Use `hermes skills install` — not `npx skills`.**

Hermes has its own built-in skill management CLI. The correct workflow:
...
The Skills CLI (`npx skills`) is the package manager for the open agent skills ecosystem — it is **not used on Hermes**. Skills on Hermes are managed via `hermes skills` commands above. The `npx skills` ecosystem commands (find, add, update) documented below apply to **other agent platforms** (Claude Code, Codex, etc.), not Hermes.
