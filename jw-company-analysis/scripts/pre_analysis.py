...
版本：v3.60.7（⚠️评分仅供参考，agent需补充：①industry_type判定②"求"框架评估③PEV/正常化PE等专用估值④管理层定性评估⑤逆向检查）
...
def step0_fetch_data(symbol: str, market: str) -> dict:
    ...
    # 确定脚本路径
    scripts_dir = Path.home() / ".hermes" / "skills" / "rancbos-skills"
  ...
  python pre_analysis.py --symbol 600519 --market A --force  # 忽略已有检查点
    ...
    if not args.force:
        ...
        if not validation["valid"]:
            print("  ⚠️ 检查点损坏，尝试修复...")
            if repair_checkpoint(checkpoint_dir):
                print("  ✅ 修复成功，继续执行")
            else:
                print("  ❌ 修复失败，从 Step 0 重新开始")
                args.force = True
...
def validate_checkpoint(checkpoint_dir: Path) -> dict:
    ...
    if not meta_path.exists():
        result["valid"] = False
        result["errors"].append("checkpoint.json 不存在")
        return result
    ...
    try:
        ...
        result["valid"] = False
        result["errors"].append(f"checkpoint.json 格式错误: {e}")
        return result
    
    # 检查必要字段
    required_fields = ["last_step", "timestamp", "files"]
    for field in required_fields:
        if field not in meta:
            result["valid"] = False
            result["errors"].append(f"checkpoint.json 缺少字段: {field}")
    
    if not result["valid"]:
        return result
    
    result["last_step"] = meta["last_step"]
    ...
    for filename in meta["files"]:
        ...
        try:
            ...
            # 检查必要字段
            if "step" not in data:
                result["warnings"].append(f"{filename}: 缺少 'step' 字段")
            if "scores" not in data and "sources" not in data:
                result["warnings"].append(f"{filename}: 缺少 'scores' 或 'sources' 字段")
                
        except json.JSONDecodeError as e:
            result["errors"].append(f"{filename}: JSON 格式错误: {e}")
            result["valid"] = False
    
    # 检查步骤连续性
    expected_steps = list(range(result["last_step"] + 1))
    actual_steps = []
    for filename in meta["files"]:
        try:
            with open(checkpoint_dir / filename, "r", encoding="utf-8") as f:
                data = json.load(f)
                if "step" in data:
                    actual_steps.append(data["step"])
        except:
            pass
    
    missing_steps = set(expected_steps) - set(actual_steps)
    if missing_steps:
        result["warnings"].append(f"缺少步骤: {sorted(missing_steps)}")
    
    return result


def repair_checkpoint(checkpoint_dir: Path) -> bool:
    """
    尝试修复损坏的检查点
    ...
    print(f"  ⚠️ 检查点有 {len(validation['errors'])} 个错误:")
    for error in validation["errors"]:
        print(f"    - {error}")
    
    # 尝试重建 checkpoint.json
    existing_steps = []
    for filepath in checkpoint_dir.glob("step*.json"):
        try:
            with open(filepath, "r", encoding="utf-8") as f:
                data = json.load(f)
                if "step" in data:
                    existing_steps.append((data["step"], filepath.name))
        except:
            pass
    
    if existing_steps:
        existing_steps.sort(key=lambda x: x[0])
        last_step = existing_steps[-1][0]
        files = [s[1] for s in existing_steps]
        ...
        print(f"  ✅ 已重建 checkpoint.json (last_step={last_step})")
        return True
    
    print("  ❌ 无法修复：没有可用的 step 文件")
    return False
