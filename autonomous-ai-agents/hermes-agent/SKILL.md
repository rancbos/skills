...
hermes plugins list/install/remove  Plugin management (pip-distributed plugins: see `references/installing-pip-plugins.md`)
...
1. `hermes plugins list` — check if plugin is registered
2. For pip-distributed plugins: ensure installed into Hermes' venv (`$HOME/.hermes/hermes-agent/venv/bin/python -m pip list | grep <name>`)
3. Check `plugins.enabled` in config.yaml has the correct entry point name
4. **Do not** rely on `hermes plugins enable` for pip-distributed plugins — edit `~/.hermes/config.yaml` directly. Full guide: `references/installing-pip-plugins.md`
5. Restart Hermes (new session or gateway restart)
...
|------|----------------|
| `references/installing-pip-plugins.md` | Installing pip-distributed Hermes plugins |
| `references/prompt-builder-environment-hints.md` | Design notes for the system prompt's environment block |
