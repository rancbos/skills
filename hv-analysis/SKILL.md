...
1. **PDF依赖安装（可选但耗时长）**：本Skill自带 `scripts/md_to_pdf.py`（基于WeasyPrint）。安装命令：`pip install weasyprint markdown --break-system-packages`。**注意：weasyprint 安装极慢（实测 10+ 分钟），且依赖 Pango/cairo 等系统库可能安装失败。如果主要用于飞书/Notion 等平台交付，跳过此步，直接走文档导入路线。**
2. **飞书文档交付路径（如需）**：如交付目标是飞书云文档，加载 `feishu-document-content` skill，按其 Import API 流程操作。Markdown → 上传Drive → 创建Import Task → 轮询 → 获取文档URL。
...
**重要：子Agent工具能力限制。** 当前环境中子Agent（delegate_task）的可用工具集与顶层agent不同——部分环境中子Agent没有真实联网能力（WebSearch返回空结果），依赖记忆库而非实时数据。需要联网的信息**不要完全委托给子Agent**，以下信息必须由主Agent直接通过terminal的curl/fetch获取：

- GitHub仓库基础数据（stars、forks、创建时间）：`curl -s "https://api.github.com/repos/{owner}/{repo}"` 
- GitHub Release列表：`curl -s "https://api.github.com/repos/{owner}/{repo}/releases?per_page=30"`
- GitHub Commit历史：`curl -s "https://api.github.com/repos/{owner}/{repo}/commits?per_page=10"`
- GitHub Contributors：`curl -s "https://api.github.com/repos/{owner}/{repo}/contributors?per_page=10"`
- README内容：直接从本地工作目录读取（`/root/.hermes/hermes-agent/README.md`），无需联网

建议分工：
- **主Agent（直接联网）**：GitHub基础数据、Release Notes、Commit历史、本地文件读取
- **子Agent 1 — 纵向信息补充**：创始人背景、发展历程叙事、行业环境背景
- **子Agent 2 — 横向信息补充**：竞品用户口碑、市场传闻、媒体报道
...
### 方案A：PDF（需要weasyprint）

报告写完后，使用本Skill自带的 `scripts/md_to_pdf.py` 脚本将Markdown转为排版精美的PDF。

**注意：weasyprint 安装极慢（实测 10+ 分钟）且依赖 Pango/cairo 系统库，可能安装失败。生产环境建议优先用方案B。**
...
使用 `feishu-document-content` skill 的 Import API：
1. 确保报告存为 `[研究对象]_横纵分析报告.md`
2. 加载 `feishu-document-content` skill 获取 API 流程
3. 获取 tenant_access_token → 上传md文件 → 创建import_task → 轮询 → 获取文档URL
4. 轮询用 4-5 秒间隔，约 1-2 分钟完成转换
5. 删除Drive中的临时md文件

### 方案C：本地保存

Markdown文件本身就是完整交付物，保存到 `~/.hermes/` 或用户指定目录即可。
...
完整的 JSON Schema 定义见 `references/schema.json`，包含纵轴/横轴/交叉洞察的字段定义和数据类型规范。
