...
ALIASES = {
    ...
    "what_it_is_not": None,
    # v4.55: "connections" is NOT an alias for "related" — it's a structured
    # Luhmann Scan field (requires/relates_to/enables_method). Do not map it.
}
...
def parse_entries(path: Path):
    ...
    for block in raw_blocks:
        ...
        for line in block.split("\n"):
            ...
            fm = re.match(r"^\s*-?\s*\**\s*([A-Za-z_][\w\-]*)\s*\**\s*:\s*(.*)", line)
            if fm:
                key = fm.group(1)
...
def validate_source_line(value: Any) -> bool:
    """Accept: positive int, numeric string, or descriptive chapter reference (e.g. '1984年致股东信', 'ch017', '第三章')."""
    ...
    if isinstance(value, str):
        ...
        if re.fullmatch(r"\d+", s):
            return int(s) > 0
        # Descriptive reference: year-based, chapter-based, or Chinese chapter names
        if re.search(r'\d{4}年|ch\d+|第[零一二三四五六七八九十\d]+[章篇节]|序|前言|附录|致股东|致合伙', s, re.I):
            return True
        # At least 2 chars and contains Chinese characters
