...
确认数据时间范围和政策文件有效期。
人工确认以下必填项完整性：行业名称、GB/T 4754-2017四位代码、地理范围、分析目的。

- ✅ 必填项齐全 → 进入步骤 1
- ❌ 缺失 → 返回缺失清单，要求用户补充

> ⚠️ 注意：`scripts/validate_industry.py` 是**报告生成后**的校验脚本（检查已生成报告的结构完整性），不是输入参数校验工具。用法：`python3 scripts/validate_industry.py <报告文件路径>`，传入生成的 .md 报告文件路径。
...
```json
{
  "skill_name": "jw-industry-analysis",
  "skill_version": "2.0.0",
...
### #6：搜索API内容审查导致数据采集失败
- **症状**：`web_search` 返回 `input new_sensitive`，搜索结果为空
- **原因**：部分搜索引擎（如 MiniMax）对特定关键词（涉及政治敏感词、金融敏感词组合）触发内容审查
- **解决**：(1) 并行发起多个搜索请求，分散风险；(2) 拆分敏感词组合（如"白酒+政策+消费税"拆为两次搜索）；(3) 改用 DuckDuckGo 搜索作为降级方案；(4) 调整关键词用同义词替代
