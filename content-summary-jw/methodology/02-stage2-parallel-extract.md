...
**最优：并行 spawn（环境支持时）**:
对 5 个 extractor 同时启动 agent，每个 agent 接收:
- 对应 extractor prompt（`extractors/<type>-extractor.md`）
- 阶段 1 的 `stage1-understanding.md` 内容
- 书籍文本（完整或分块路径）
- 产出文件路径（`candidates/<type>.md`）

**回退：串行执行（环境不支持并行时）**:
按 `framework → principle → case → counter-example → glossary` 顺序依次运行，每个 extractor 完成即写入 `candidates/`，不等待其他 extractor 结果。**关键约束**: 串行模式下仍不得读取其他 extractor 的产出，以保持 V1 跨域验证的独立性。

**长书分块并行**: 先按章节切块，每块并行运行 5 个 extractor（或串行回退），最后合并去重形成统一的 `candidates/`。
