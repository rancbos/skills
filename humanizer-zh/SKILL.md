...
### Pitfall 0：润色时反而加重 AI 痕迹

当用户要求"润色""优化"中文文本时，agent 的输出反而比用户原文更像 AI。这是最常见的失败模式。

**典型表现：**
- 用户原文："先定标准。训练场里不再靠师傅'口传心授'……"
- Agent 润色后："围绕'定标准、建闭环、通数据'三条主线……"（三字口号 + 三段式）

**根因：** Agent 倾向于把口语化原文"升级"为更正式、更对仗的表达，这恰好是 AI 痕迹。

**规则：** 润色中文文本时，改动幅度应小于你的直觉。用户原文的口语感、不齐整、不完美正是人味。任务是"去掉明显的口语粗糙"，不是"升级为书面语"。
...
处理中文文本时，除了上述通用模式，还需检查中文特有的 AI 痕迹。详见 `references/chinese-specific-patterns.md`，包含 11 种中文特有模式和中文版快速检查清单。

关键模式速记：
- 四字/六字节奏陷阱（列表标题字数齐整）
- "无人X"重复（没人管/没人问并列）
- 三字口号式分组（定标准、建闭环、通数据）
- 管理黑话堆砌（提效/赋能/抓手/闭环/打通）
- "不仅仅…更是…"否定式排比
- 通用积极结尾（持续优化、不断完善）
...
- `references/management-writing-example.md` — 中文管理创新项目润色实战案例（Pitfall 0 过度润色的完整对比 + 6种常见AI模式 + 润色原则）
